...
namespace HK.Formas
{
    public partial class FrmRestaurantPagar : Form
    {
        private double? montoPagar = 0;
        public Pago pago { set; get; }
        public Tercero cliente { set; get; }
        public MesasAbierta mesaAbierta { set; get; }
        public FrmRestaurantPagar()
        {
            InitializeComponent();
        ...
        {
            if (sender == this.tarjetaCRTextEdit || sender == this.tarjetaDBTextEdit)
            {
                mesaAbierta.Totalizar(10);
            }
            else
            {
                mesaAbierta.Totalizar();
            }
            this.montoPagar = mesaAbierta.MontoTotal;
            DevExpress.XtraEditors.CalcEdit editor = (DevExpress.XtraEditors.CalcEdit)sender;
        ...
        {
            mesaAbierta.Totalizar(OK.SystemParameters.TasaIvaB.GetValueOrDefault());
            this.montoPagar = mesaAbierta.MontoTotal;
            LimpiarPagos();
            pago.CestaTicket = pago.MontoPagar;
        ...
        {
            if (mesaAbierta.MontoTotal < 200000 && !this.CedulaRifButtonEdit.Text.Contains("J"))
            {
                mesaAbierta.Totalizar(OK.SystemParameters.TasaIvaB.GetValueOrDefault());
            }
            else
            {
                mesaAbierta.Totalizar(OK.SystemParameters.TasaIva.GetValueOrDefault());
            }
            this.montoPagar = mesaAbierta.MontoTotal;
            LimpiarPagos();
            pago.TarjetaCredito = pago.MontoPagar;
            tarjetaCRTextEdit.SelectAll();
            this.tarjetaCRTextEdit.Focus();
            Totalizar();
        }
        ...
        private void PagarTarjetaDB()
        {
            ...
            this.montoPagar = mesaAbierta.MontoTotal;
            LimpiarPagos();
            pago.TarjetaDebito = pago.MontoPagar;
            tarjetaDBTextEdit.SelectAll();
            this.tarjetaDBTextEdit.Focus();
            Totalizar();
        }
